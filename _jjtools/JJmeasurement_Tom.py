...
import numpy as np
from QCmeasurement_Tom import *
from meas_util import *
from tqdm import tqdm, tqdm_notebook

from Exps_Tom import *
...
#             self.isexp = False
    ...
    def IVC_cust_BT (self, i_list, Ioff = 0, dt = 0.1, N_avg = 1, label = '', B_list = None, T_list = None):
...
#         V.Voff = 0
        ...
        meas = self.set_meas(V, I, B)

        #tB_list = tqdm_notebook(B_list, leave = False)
        ti_list = tqdm_notebook(i_list, leave = False)
        ...
        def isweep():
            for i in ti_list:
                ...
                is_vs = [[I.get(),V.get(),B.get()] for _ in range( N_avg)]
                ir, v, b = np.mean(is_vs, axis = 0)
#                 if B_list is None:
#                     res = [( I, ir - Ioff  ), ( V, v - Voff  )]
#                 else:
                res = [( I, ir - Ioff  ), ( V, v - Voff  ), (B, b)]#do we need to use a B.get() or is the setpoint good enough?]
                #ti_list.set_description('I = {}A'.format( SI(ir) ))
        ...
        with meas.run() as datasaver:
            if B_list is None:            
                ...
                tB_list = tqdm_notebook(B_list, leave = False)
                for b in tB_list:
                    # Do we need to ensure we're always approaching B from the same direction? (magnet hysteresis etc)
                    B.set(b)
                    ti_list = tqdm_notebook(i_list, leave = False)
...
#             self.isexp = False
    ...
    def IVC_udu (self, amp, stp, **kwargs):
        ...
        run_id = self.IVC_cust_BT ( i_list, **kwargs)
...
#     def condense_old_Bscan(exp_ids):
#         return
