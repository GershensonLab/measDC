...
#         self.setup = lambda : None
    ...
    def set_meas(self, dep, fast_indep, slow_indep1 = None, slow_indep2 = None):
        """
        ...
        if slow_indep1 is not None and slow_indep2 is not None:
            meas.register_parameter( slow_indep1 )  # register the first independent parameter
            meas.register_parameter( slow_indep2 )
            meas.register_parameter( dep , setpoints = ( slow_indep1, slow_indep2,  fast_indep ) ) 
        elif (slow_indep1 is not None and slow_indep2 is None):
            meas.register_parameter( slow_indep1 )
            meas.register_parameter( dep , setpoints = ( slow_indep1,  fast_indep ) ) 
        elif (slow_indep2 is not None and slow_indep1 is None):
            meas.register_parameter( slow_indep2 )
            meas.register_parameter( dep , setpoints = ( slow_indep2,  fast_indep ) ) 
        else:
    ...
    def mock_meas_BT(self, y : tuple, x : tuple, x1 = None, x2 = None, label = 'description'):
        ...
        meas = self.set_meas(ydev, xdev, x1dev, x2dev)
        self.name_exp( exp_type = label)

        with meas.run() as datasaver:

            for _x, _y, _x1, _x2 in zip(xdat, ydat, x1dat, x2dat):

                res = [( xdev, _x ), ( ydev, _y  ), ( x1dev, _x1 ), ( x2dev, _x2 )]
                datasaver.add_result(*res) 

        return datasaver, datasaver.run_id 
